...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default="-1",
        # choices=["-1", "0", "uni"],
        help=("Encoder timestep scheduler type."),
    )
...
def main(args):
    ...
    train_flip = transforms.Lambda(lambda x: x) if args.no_hflip else transforms.RandomHorizontalFlip(p=0.5)
    ...
    def preprocess_train(examples):
        ...
        for col_name in ['image_0_uid',	'image_1_uid']:
            ...
            images = [image.resize((1024, 1024)) for i, image in enumerate(images)]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                need_timestep = args.timestep_type
                ...
                if need_encoder and need_timestep == '-1':
                ...
                elif (need_encoder and (need_timestep == '0' or need_timestep == 'uni')) or need_clip:
                    # Previous timestep to use in encoder unet
                    timesteps_prev = torch.zeros_like(timesteps)

                    # Compute x_0 latents using predicted and actual noise,
                    # timesteps_prev and variance_noise are unused (see function)
                    variance_noise = torch.randn(
                        (bs, c, h, w), dtype=latents.dtype, device=latents.device
                    ).repeat(2, 1, 1, 1)
                    noisy_latents_prev_pred = ddpm_scheduler_step_batched(
                        ...
                        noisy_latents,
                        variance_noise
                    ).pred_original_sample.to(latents.dtype)
                    if need_ref:
                        ref_noisy_latents_prev_pred = ddpm_scheduler_step_batched(
                            noise_scheduler,
                            ref_noise_pred,
